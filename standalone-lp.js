(async()=>{
  ...
  override.textContent=`
    *{box-sizing:border-box}html{background:#e7eeec}body{margin:0;overflow-x:hidden;background:#e7eeec;color:#17383e;-webkit-font-smoothing:antialiased}
    ...
    .standalone-shell[data-lp-format="survey"] .question-block{counter-increment:lpquestion;margin:30px -8px 0!important;padding:34px 16px 28px!important;border:0!important;border-radius:2px!important;background:linear-gradient(160deg,#fff 0 72%,#edf7f5 72%)!important;box-shadow:0 12px 34px rgba(18,63,66,.15)!important;outline:1px solid #c8dcda}
    .standalone-shell[data-lp-format="survey"] .question-block::before{content:"Q" counter(lpquestion);position:absolute;left:-6px;top:-18px;display:grid;place-items:center;width:62px;height:62px;border:4px solid #fff;border-radius:50%;background:var(--lp-ink);color:#fff;font:900 23px/1 Georgia,serif;box-shadow:0 5px 0 var(--lp-gold),0 8px 18px rgba(18,54,57,.18);z-index:3}
    .standalone-shell[data-lp-format="survey"] .question-block::after{content:"あなたの回答をタップ";right:12px;top:10px;color:#b99228;font:900 10px/1.2 Arial,sans-serif;letter-spacing:.08em}
    .standalone-shell[data-lp-format="survey"] .question-no{margin-left:48px;padding:5px 12px!important;background:transparent!important;color:#8b6a15!important;border:1px solid #d7bb64;border-radius:999px!important;font-size:10px!important}
    .standalone-shell[data-lp-format="survey"] .question-block h2{margin-top:18px!important;font-size:24px!important;line-height:1.5!important;text-align:center;color:var(--lp-ink)}
