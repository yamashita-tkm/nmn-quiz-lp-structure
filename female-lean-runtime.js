(async()=>{
  ...
  const source=await fetch('female-lean-base.html?standalone=20260818-proof2v1',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('LP source unavailable');return r.text()});
  ...
  override.textContent=`
    *{box-sizing:border-box}html{background:#f3f8f7}body{margin:0;overflow-x:hidden;background:#f3f8f7;color:#315b5b;-webkit-font-smoothing:antialiased}
    ...
    .standalone-shell .official-site-footer{position:relative;margin:0 -20px;padding:58px 20px 0;background:#fff;color:#111;text-align:center}
  ...
  const variant=host.dataset.lpVariant||'';
  const boot=`\n audience=${JSON.stringify(audience)};format=${JSON.stringify(format)};step=0;answers=[];document.querySelector('.device').dataset.audience=audience;document.querySelector('.device').dataset.format=format;document.querySelector('.device').dataset.variant=${JSON.stringify(variant)};renderIntro();`;
  Function(`var femaleLean=${JSON.stringify(variant==='female-lean')};\n`+script.textContent+boot)();
